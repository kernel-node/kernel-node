...
pub struct PeerManager {
    ...
    // Held so stop() can shut connections without waiting for threads to time out.
...
impl PeerManager {
        ...
        for i in 0..self.max_peers {
            ...
            let handle = thread::spawn(move || {
                ...
                while running.load(Ordering::SeqCst) {
                    ...
                    let peer = BitcoinPeer::new(i, socket_addr, network, &node_state);
    ...
    // Call stop() first; otherwise blocks until all peer threads time out.
    pub fn join(self) {
